...
## How it Works
1. This action reads a code coverage report in cobertura format. For example generated by the Coverlet package for .NET. 
...
## Requirements
For this action to work there must be an cobertura.xml file available in the workflow and a path to it must be specified as an input parameter.

Making this cobertura.xml in .NET is really simple. All you need to do is to install the nuget package ```coverlet.msbuild``` and it's dependency ```coverlet.collector``` in your test project. Then you can generate the test coverage file during your test execution with this command:

```
dotnet test  -p:CollectCoverage=true -p:CoverletOutput=TestResults/ -p:CoverletOutputFormat=cobertura
```

The above command will generate an cobertura report in ```TestResults/coverage.cobertura.xml```. 

You don't necessarily have to use the above example to generate the cobertura report. If you have other means of doing this, then that should not cause any problems. You actually don't even need a .NET solution. As long as you can provide a path for the coverage file. 
...
| path            | Required  | The path to the cobertura.xml file |
...
jobs:
  build:
    ...
    - name: Test
      run: dotnet test  -p:CollectCoverage=true -p:CoverletOutput=TestResults/ -p:CoverletOutputFormat=cobertura --no-build --verbosity normal <MyProject>.Tests/
    - name: Create Test Coverage Badge
      ...
      with:
        ...
        color: brightgreen
        path: <MyProject>.Tests/TestResults/coverage.cobertura.xml
